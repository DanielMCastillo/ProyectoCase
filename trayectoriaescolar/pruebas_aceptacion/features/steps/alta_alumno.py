...
def step_impl6(context, contra, recontra):
    context.driver.find_element(By.ID, 'examplePasswordInput').send_keys(contra)
    context.driver.find_element(By.ID, 'exampleRepeatPasswordInput').send_keys(recontra)
    
@when(u'oprimo el botón Guardar Alumno')
def step_impl7(context):
    context.driver.find_element(By.XPATH, '//*[@id="content"]/div/div[2]/div[2]/div[2]/div/div[1]/div[2]/form/button').click()
    time.sleep(1)
...
@then(u'me muestra el error "{esperado}"')
